...
;(function () {
  ...
  function initialize() {
    h1 = document.querySelector('.fof>h1')
    canvas = document.querySelector('.fof>canvas')

    if (canvas) {
      canvas.width = DISPLAY_WIDTH
      canvas.height = DISPLAY_HEIGHT - h1.clientHeight - 15
...
function Eye(canvas, x, y, scale, time) {
  ...
  this.render = function (mouse) {
    ...
    // The lower the factor, the greater the swing
    const irisSwingFactor = 1.7

    // Offset the iris depending on mouse position
    const eio = {
      x:
        (mouse.x - ei.x / irisSwingFactor) /
        (window.innerWidth - ei.x / irisSwingFactor),
